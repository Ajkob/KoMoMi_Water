...
/**************************************************************
  ...
  Design by KoMoMi  8 Mar 2018 (LAST CHK)
  VERSION : 0.4.0ST4
 **************************************************************/
//SCL as D1/GPIO5
//SDA as D2/GPIO4
...
String sw_ver = "Version 0.4.0ST4";
...
void setTable() {
  ...
  Serial.print(msg);
  for (int i = 0; i < sizeof(h_Time) / sizeof(int); i++) {
    h_Time[i] = server.arg(i).toInt();
    Serial.print(String(h_Time[i]) + ", ");
    //t_htime[i] = str.toInt();

  }
  for (int i = 6; i < server.args(); i++) {
    k_time[i-6] = server.arg(i).toInt();
    Serial.print(String(k_time[i-6]) + ", ");
    //t_htime[i] = str.toInt();
...
boolean chk_work() {
  ...
  for (int i = 0; i < sizeof(h_Time) / sizeof(int); i++) {
    if (HH == h_Time[i]) {
      Serial.print("The working time is active at ");
      Serial.println(String(h_Time[i]));
      state = true;
    }
  }
  if (state)
...
void automatic() {
  int numZone = 4;// Number of Zoning
  Serial.println("***  Auto Starting  *****");
  closeAll();
  Serial.println("--auto--");
  lcd.setCursor(12, 0);
  lcd.print("Auto   ");
  String record[numZone];
  while (autoMode) //do if still auto mode
  {
    soilsensor();
    lcd.setCursor(12, 0);
    lcd.print("Auto   ");
    if (chk_work() && !sensorChk)  //if work hour is valid and soil moisture is valid
    {
      //bool stop = false;
      for (int i = 0; (i < numZone) && !stop; i++) {
        Serial.print(" i is : ");
        Serial.print(i);
      ...
      {        
        ...
        if (k > 0) //if time operate is set more than 0 do this
        {
          ...
          { 
            ...
            lcd.setCursor(12, 0);
            lcd.print("Auto   ");
            delay(700); //adjust time for 1 minute delay
            ...
            Serial.println("*** The dulation is zero ***");
            check(record);
        }
...
void soilsensor() {
  ...
  int rawMoist = analogRead(sensorPin);
  //moisture = map(rawMoist,0,1023,100,0);
    moisture = map(rawMoist,500,1023,100,0);
  //Read analog value of the pin A0 which is between 0 (Totally Wet) and 1023 (Totally Dry) 
  //Then, convert it to value between 0 and 100 
  //Next, store it into the variable 'moisture'  
...
/*    
  ...
  int moistureV2 = (100-(moisture)*100/1024);
  Serial.print("Soil Moisture Raw data = ");
  Serial.println(moisture);
  Serial.print("Moisture of Soil: ");
  Serial.print(moistureV2);
  Serial.println(" %");  
  //delay(500);
 */ 
}
